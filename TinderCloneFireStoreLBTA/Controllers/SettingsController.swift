...
class SettingsController: UITableViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate{
    ...
    fileprivate func fetchCurrentUser() {
        Firestore.firestore().fetchCurrentUser { (user, error) in
            if let err = error{
                print("Could not load current user", err)
                return
            }
    ...
    static let defaultMaxSeekingAge = 22
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 5{
            ...
            //            ageRangeCell.minSlider.addTarget(self, action: #selector(handleNumEqual), for: .valueChanged)
            //            ageRangeCell.maxSlider.addTarget(self, action: #selector(handleNumEqual), for: .valueChanged)
            
            //gotta setup the labels on our cell
            ...
            ageRangeCell.minSlider.value = Float(user?.minSeekingAge ?? SettingsController.defaultMinSeekingAge)
            ageRangeCell.maxSlider.value = Float(user?.maxSeekingAge ?? SettingsController.defaultMaxSeekingAge)
